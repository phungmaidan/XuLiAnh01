...
public:
	...
	Tham số
		...
		- Vị trí của kênh màu ảnh đích đang xét
	Trả về
		- Giá trị màu được nội suy
	*/
	virtual uchar Interpolate(
		float tx, float ty, uchar* pSrc, int srcWidthStep, int nChannels, int iChannel_dst) = 0;
	PixelInterpolate();
	~PixelInterpolate();
};
...
{
public:
	uchar Interpolate(float tx, float ty, uchar* pSrc, int srcWidthStep, int nChannels, int iChannel_dst);
	BilinearInterpolate();
	~BilinearInterpolate();
};
